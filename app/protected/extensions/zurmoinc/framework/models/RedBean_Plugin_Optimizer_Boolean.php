<?php
    ...
    /**
     * A Boolean only version of RedBean's existing RedBean_Plugin_Optimizer_Datetime.
     ...
     * so there is no point in repeating it even though the boolean optimizer should not
     ...
     */
    class RedBean_Plugin_Optimizer_Boolean extends RedBean_Plugin_Optimizer_Datetime
    {
        public function optimize()
        {
            try
            {
                ...
                if (array_key_exists($this->column, $columnNamesToTypes))
                {
                    ...
                    {
                        $this->adapter->exec("alter table {$this->table} change {$this->column} {$this->column} tinyint(1)");
                    }
                }
                else
                {
                    $this->adapter->exec("alter table {$this->table} add {$this->column} tinyint(1)");
                }
            }
